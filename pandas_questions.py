...
def merge_regions_and_departments(regions, departments):
    ...
    results = results.rename(columns={'code_x': 'code_reg',
                                      'name_x': 'name_reg',
                                      'code_y': 'code_dep',
                                      'name_y': 'name_dep'})
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    referendum_metro['Department code'] = (
        referendum_metro['Department code'].apply(
            lambda x: x.zfill(2) if x not in ['2A', '2B'] else x
        )
    )

    regions_and_departments_metro = regions_and_departments[
        ~regions_and_departments['code_reg'].isin(
            ['01', '02', '03', '04', '06', 'COM']
        )
    ]
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing
    ...
    referendum_with_geo['ratio'] = (
        ...
        (referendum_with_geo['Registered'] -
         referendum_with_geo['Abstentions'] -
         referendum_with_geo['Null'])
    )
